...
### 遇到的問題：
* 在處理 Bonus 要求的100x100矩陣的測資，在第一次執行時，L2範數的誤差來到了接近10^14，誤差高得離譜。但是在先前幾次簡單的測資，儘管有誤差，也僅僅只有10^-6。為了探討這項問題，我向老師請教過，但是在實際操作並執行時，我先用二分慢慢找，但每組的誤差依舊非常大，所以我嘗試用了另一組測資10x10矩陣，先控制好解答，並反向推回一個矩陣在執行，發現在小數點後第七位發生浮點數不精確(？)的問題，所以我在 `gauss.py` 多新增直接設定為 0，之後再測試一次100x100
